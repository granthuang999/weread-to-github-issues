...
export async function refreshSession(currentCookie: string): Promise<string> {
  ...
  try {
    const headers = getShelfHeaders(currentCookie); // 使用网页 Headers
...
export async function getBookshelfBooks(cookie: string): Promise<any[]> {
    ...
    try {
        const headers = getShelfHeaders(cookie); // 关键修正：使用 getShelfHeaders
...
export async function getBookInfo(
  ...
  try {
    const headers = getApiHeaders(cookie); // 关键修正：使用 getApiHeaders
